...
newdata = pd.merge(x_train,y_train , left_index=True, right_index=True)
...
#Broullion
from sklearn.model_selection import KFold, validation_curve, cross_val_score, train_test_split, GridSearchCV
X_train1, X_test1, y_train1, y_test1 = train_test_split(x_train_pca, y_train, test_size=0.33, random_state=42)

cph = CoxPHFitter()

v = cross_val_score(CoxPHFitter(),x_train_pca,y_train,scoring='concordance_index',cv=5).mean()

gh = GridSearchCV(cph,param_grid={'alpha':np.linspace(0,2,100)},scoring="concordance_index",cv=5)
...
grid_search.fit(x_train_pca,y_train['SurvivalTime'])
...
def feature_selection(X,y):
    ...
    for j in X.columns:
        Xj = X[j]
        temp = pd.merge(Xj,y, left_index=True, right_index=True)
        m.fit(temp,duration_col='SurvivalTime')
        scor[j] = m.score_
    return scor

scores = feature_selection(x_train_pca, y_train)
